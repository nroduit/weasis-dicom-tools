...
public class CGetForward implements AutoCloseable {
  ...
  //                            new AttributeEditorContext(tsuid,
  ...
  //                        if (attributesEditor != null && attributesEditor.apply(attributes,
  // context)) {
  ...
  //                            as.abort();
  //                            throw new AbortException("DICOM associtation abort. " +
  // context.getAbortMessage());
  //                        }
  //
  ...
  //                    ServiceUtil.notifyProgession(streamSCU.getState(),
  // rq.getString(Tag.AffectedSOPInstanceUID),
  //                        rq.getString(Tag.AffectedSOPClassUID), Status.ProcessingFailure,
   ...
   * @throws IOException
   */
  //    public CGetForward(AdvancedParams forwardParams, DicomNode callingNode, DicomNode
  // destinationNode,
  ...
  //    }
  //
  //    public final void setInformationModel(InformationModel model, String[] tss, boolean
  // relational) {
  //        this.model = model;
  ...
  //        }
  //        rq.addPresentationContext(new PresentationContext(2 *
  // rq.getNumberOfPresentationContexts() + 1, cuid, tsuids));
  //    }
  //
  //    public void open()
  ...
  //    }
  //
  //    private void retrieve(Attributes keys, DimseRSPHandler rspHandler) throws IOException,
  ...
  private static DicomState process(
    ...
    //        try {
    //            forward = new CGetForward(forwardParams, callingNode, destinationNode, progress,
    // attributesEditor);
    ...
    //            } else {
    //                throw new IllegalArgumentException(queryRetrieveLevel + " is not supported as
    ...
    //                String timeMsg =
    //                    MessageFormat.format("DICOM C-GET connected in {2}ms from {0} to {1}. Get
    // files in {3}ms.",
    //                        forward.getAAssociateRQ().getCallingAET(),
    // forward.getAAssociateRQ().getCalledAET(), t2 - t1,
    //                        t3 - t2);
  ...
  //                configureStorageSOPClass(getSCU, (String) entry.getKey(), (String)
  // entry.getValue());
  //            }
  //        } catch (Exception e) {
