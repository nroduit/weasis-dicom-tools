...
public class IOD extends ArrayList<IOD.DataElement> {
  ...
  public static class DataElement implements Serializable {
    ...
    public DataElement(
        int tag, VR vr, DataElementType type, int minVM, int maxVM, int valueNumber) {
      this.tag = tag;
      this.vr = vr;
      this.type = type;
      this.minVM = minVM;
    ...
    public DataElement setValues(String... values) {
      if (vr == VR.SQ) throw new IllegalStateException("vr=SQ");
      this.values = values;
      return this;
    }
    ...
    public DataElement setValues(Code... values) {
      if (vr != VR.SQ) throw new IllegalStateException("vr=" + vr);
      this.values = values;
      return this;
    }
  ...
  public abstract static class Condition {
    ...
    public abstract boolean match(DicomObject attrs);
  ...
  public static class And extends CompositeCondition {
    ...
    public boolean match(DicomObject attrs) {
      for (Condition child : childs) {
        if (!child.match(attrs)) return not;
      }
      return !not;
    }
  }
  ...
  public static class Present extends Condition {
    ...
    public boolean match(DicomObject attrs) {
      return false;
      //   return not ? !item(attrs).containsValue(tag) : item(attrs).containsValue(tag);
    }

    protected DicomObject item(DicomObject attrs) {
      for (int sqtag : itemPath) {
        if (sqtag == -1) {
          attrs =
              (sqtag == -1) ? attrs.getParent().orElse(null) : attrs.get(sqtag).get().getItem(0);
        }
      }
      return attrs;
    }
  ...
  public static class MemberOf extends Present {
    ...
    public boolean match(DicomObject attrs) {
      ...
      DicomObject item = item(attrs);
      if (item == null) return matchNotPresent;

      if (values instanceof int[])
        return not ? !match(item, ((int[]) values)) : match(item, ((int[]) values));
      else if (values instanceof Code[])
        return not ? !match(item, ((Code[]) values)) : match(item, ((Code[]) values));
      else return not ? !match(item, ((String[]) values)) : match(item, ((String[]) values));
    }

    private boolean match(DicomObject item, String[] ss) {
      String val = item.getString(tag).orElse(null);
    ...
    private boolean match(DicomObject item, Code[] codes) {
      //            Sequence seq = item.getSequence(tag);
      //            if (seq != null)
      //                for (DicomObject codeItem : seq) {
      //                    try {
    ...
    private boolean match(DicomObject item, int[] is) {
      int val = item.getInt(tag, Integer.MIN_VALUE).orElse(Integer.MIN_VALUE);
      if (val == Integer.MIN_VALUE) return matchNotPresent;
      for (int i : is) {
  ...
  private static class SAXHandler extends DefaultHandler {
    ...
    private Present present(org.xml.sax.Attributes atts) throws SAXException {
      ...
      return new Present(
          tagPath[lastIndex],
          lastIndex > 0 ? Arrays.copyOf(tagPath, lastIndex) : DicomElement.EMPTY_INTS);
    }
    ...
    private void startCode(
        ...
        String codeMeaning)
        throws SAXException {
      if (codeValue == null) throw new SAXException("missing codeValue attribute");
      if (codingSchemeDesignator == null)
        throw new SAXException("missing codingSchemeDesignator attribute");
      if (codeMeaning == null) throw new SAXException("missing codeMeaning attribute");
      //     codes.add(new Code(codeValue, codingSchemeDesignator, codingSchemeVersion,
      // codeMeaning));
    }
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
      if (processCharacters) sb.append(ch, start, length);
    }
    ...
    private void startDataElement(
      ...
      if (locator != null) el.setLineNumber(locator.getLineNumber());
      iod.add(el);
    ...
    private DataElementType typeOf(String s) throws SAXException {
      if (s == null) throw new SAXException("missing type attribute");
      try {
    ...
    private int[] parseInts(List<String> list) {
      ...
      for (int i = 0; i < is.length; i++) is[i] = Integer.parseInt(list.get(i));
      return is;
    }
    ...
    private void startItem(String id, String idref, String type) throws SAXException {
      IOD iod;
      if (idref != null) {
        if (type != null) throw new SAXException("<Item> with idref must not specify type");
        ...
        iod = new IOD();
        if (type != null) iod.setType(typeOf(type));
        if (locator != null) iod.setLineNumber(locator.getLineNumber());
      }
      getLastDataElement().addItemIOD(iod);
      iodStack.add(iod);
      if (id != null) id2iod.put(id, iod);
    ...
    private void startIf(String id, String idref) throws SAXException {
      if (!conditionStack.isEmpty()) throw new SAXException("unexpected <If>");

      Condition cond;
      if (idref != null) {
        cond = id2cond.get(idref);
        if (cond == null) throw new SAXException("could not resolve <If idref:\"" + idref + "\"/>");
      } else {
        cond = new And().id(id);
      }
      conditionStack.add(cond);
      if (id != null) id2cond.put(id, cond);
      this.idref = idref;
    }
