    ...
    public static class Present extends Condition {
        ...
        protected DicomObject item(DicomObject attrs) {
            for (int sqtag : itemPath) {
                if (sqtag == -1) {
                    attrs = (sqtag == -1) ? attrs.getParent().orElse(null) : attrs.get(sqtag).get().getItem(0);
                }
            }
            return attrs;
        }
...
//                throw new IllegalStateException("vr=" + vr);
        ...
        public boolean match(DicomObject attrs) {
            ...
            if (values instanceof int[])
                ...
                return not ? !match(item, ((String[]) values)) : match(item, ((String[]) values));
        }
    ...
    private static class SAXHandler extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, org.xml.sax.Attributes atts)
            ...
            switch (qName) {
                ...
                case "Code":
                    ...
                    break;
                case "DataElement":
                    startDataElement(atts.getValue("tag"), atts.getValue("vr"), atts.getValue("type"),
                        atts.getValue("vm"), atts.getValue("items"), atts.getValue("valueNumber"));
                    break;
                case "If":
        ...
        private MemberOf memberOf(org.xml.sax.Attributes atts) throws SAXException {
            ...
            return new MemberOf(tagPath[lastIndex], vrOf(atts.getValue("vr")),
                valueNumberOf(atts.getValue("valueNumber"), 1) - 1, matchNotPresentOf(atts.getValue("matchNotPresent")),
                lastIndex > 0 ? Arrays.copyOf(tagPath, lastIndex) : DicomElement.EMPTY_INTS);
        }
